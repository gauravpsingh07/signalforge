...
def main() -> int:
    ...
    for index in range(args.baseline_count):
        ...
        events.append(
            {
                "eventId": f"evt_baseline_{int(time.time() * 1000)}_{index}",
                ...
                "statusCode": 200,
                "latencyMs": 120,
                "metadata": {"script": "generate_error_spike", "phase": "baseline"},
            }
        )
    for index in range(args.spike_count):
        events.append(
            {
                "eventId": f"evt_error_spike_{int(time.time() * 1000)}_{index}",
                ...
                "metadata": {"script": "generate_error_spike", "phase": "spike"},
            }
        )
