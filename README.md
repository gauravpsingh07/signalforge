...
Phase 0 establishes the monorepo foundation only. It includes a SvelteKit frontend shell, a FastAPI service with a health endpoint, an importable Python worker skeleton, local infrastructure configuration, documentation placeholders, and CI.
...
Phase 0 does not implement those later capabilities yet. The repository structure and docs are prepared so each phase can add them without turning the project into a dashboard-only app.
...
| --- | --- | --- |
| Frontend | SvelteKit, TypeScript, Tailwind CSS | Skeleton routes and styling |
| Backend API | FastAPI, Pydantic settings | Health route and route modules |
| Worker | Python | Importable local polling placeholder |
| Metadata DB | PostgreSQL/Neon | Migration skeleton |
...
2. Phase 1: Auth, projects, and hashed API keys.
3. Phase 2: Event ingestion, validation, rate limits, and queue abstraction.
...
- Later phases must hash ingestion API keys and show raw keys only once.
